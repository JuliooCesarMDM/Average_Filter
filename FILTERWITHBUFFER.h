...
template <class T, uint8_t FILTER_SIZE>class FilterWithBuffer : public Filter<T> {
  public:
    FilterWithBuffer();
...
template <class T, uint8_t FILTER_SIZE>void FilterWithBuffer<T, FILTER_SIZE>::Reset() {
  Filter<T>::Reset();
...
template <class T, uint8_t FILTER_SIZE>T FilterWithBuffer<T, FILTER_SIZE>::Apply(T sample) {
